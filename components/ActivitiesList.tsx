...
export const ActivitiesList = ({activityTypes, snackBar, setSnackBar, showDialog, icon} : Props) => {
    ...
    activityTypesWithDates.sort((a,b) => {
        return (a.addedAt as Timestamp).toMillis() - (b.addedAt as Timestamp).toMillis();
    });
    return <>
    <List.Section>
        {activityTypesWithoutDates.concat(activityTypesWithDates).map((activity, index) => (
            <View key={index}>
    ...
    <Snackbar
        ...
        style={snackBar.error ? { backgroundColor: coreTheme.colors.error } : { backgroundColor: coreTheme.colors.success }}
    >
        {snackBar.message}
    </Snackbar>
    </>
